...
const PastCarComponetBox = ({ isAvailable }: { isAvailable: boolean }) => {
  return (
    <div className="w-[288px] bg-[#f5fafe] h-auto rounded-[6px] shadow-lg border-[1px] border-blue_light relative">
      {!isAvailable ? (
        <div>
          <div className="w-full h-full bg-slate-700 absolute top-0 left-0 opacity-40 flex items-center justify-center"></div>
        </div>
      ) : null}
      ...
      <div className=" w-full h-auto px-2.5 py-1.5">
        ...
        <div className=" w-full h-[44px] flex justify-between items-center bg-cysdfan-500 px-1">
          <div className=" flex flex-col items-start justify-between font-semibold w-auto h-full">
            <div className="flex items-center justify-center font-semibold ">
              {" "}
              <TiTimesOutline className="text-[22px] text-color_main" />
        ...
        <button className="w-full h-[36px] text-color_main transition-all duration-200 border-[2px] border-color_main text-[14px] items-center justify-center mt-1 rounded-[6px] button_main hover:bg-color_main hover:text-white hover:scale-[.98]">
          {`${!isAvailable ? "Not Available" : "Rent Again"}`}
        </button>
      </div>
    </div>
  );
...
export default PastCarComponetBox;
