    ...
    <>
      ...
      <div className="flex items-center justify-between max-md:justify-evenly flex-row flex-wrap gap-4 mt-[10px] max-md:w-full h-auto mx-auto p-1">
        {boxArray?.map((item, index) => (
          <span
            className={`col-span-1 w-[130px] max-lg:w-[160px] max-md:max-w-[150px] h-[86px] max-lg:h-[74px] bg-white flex items-center justify-center flex-col gap-1 rounded-[6px] max-lg:rounded-[4px] cursor-pointer  border hover:border-blue_deep_light hover:scale-105 transition-all duration-200 ${
              isShadow ? "shadow-lg" : "shadow-sm border-blue_deep_light"
            }`}
            key={index}
          >
            <Image src={item.logo} width={80} height={80} alt="car_brand" />
            <span className=" font-[poppins] font-semibold mx-auto">
              {item.title}
            </span>
          </span>
        ))}
